...
# --- MongoDB Connection ---
# আপনার দেওয়া URI ব্যবহার করা হয়েছে
...
def get_config():
    ...
    # Ensure all required keys exist to avoid 500 errors
    ...
    <body class="flex flex-col md:flex-row min-h-screen">
        ...
        <script>
            function addL(){{
                ...
                d.innerHTML = `<input type="text" name="l_name[]" placeholder="Button Label (e.g. GD To)" required class="text-sm"><input type="text" name="l_url[]" placeholder="Destination URL" required class="text-sm">`;
                b.appendChild(d);
            }}
            addL();
        </script>
        ...
        <main class="flex-grow p-6 md:p-12 space-y-10">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <form method="POST" class="glass p-8 rounded-[3rem] space-y-4 shadow-2xl border-t-4 border-blue-600">
                    ...
                    <div class="space-y-1"><label class="text-[10px] uppercase font-bold text-slate-500">Telegram/Channel Link</label><input type="text" name="channel_link" value="{{{{conf.channel_link}}}}"></div>
                ...
                <div class="space-y-10">
                    ...
                    <div class="glass p-8 rounded-[3rem] space-y-6 shadow-2xl border-t-4 border-green-600">
                        <h3 class="text-green-500 font-black uppercase text-xs tracking-widest border-b border-slate-800 pb-3">Genre / Category Manager</h3>
                        <form method="POST" class="flex gap-2"><input type="text" name="cat_name" placeholder="New Genre Name" required><button name="add_cat" class="bg-green-600 px-6 rounded-xl font-black text-white">ADD</button></form>
            ...
            <form method="POST" class="glass p-10 rounded-[3.5rem] space-y-8 shadow-2xl border-t-4 border-yellow-500">
                <h3 class="text-yellow-500 font-black uppercase text-xs tracking-widest border-b border-slate-800 pb-3 italic">Monetization & Ad Scripts (7 SLOTS)</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="{{{{k}}}}" rows="4" placeholder="Paste your ad code here...">{{{{v}}}}</textarea>
                    </div>
                    {{% endfor %}}
                </div>
                <button name="update_ads" class="bg-yellow-600 w-full py-5 rounded-[2rem] font-black uppercase tracking-tighter text-xl shadow-2xl border-b-4 border-yellow-900">🚀 Update Global Ad System</button>
            </form>
        </main>
    </body></html>
    """
...
if __name__ == '__main__':
    # Vercel-এ বিল্ড করার সময় এই ব্লকটি কার্যকরী হবে না, 
    # কিন্তু লোকাল ডিব্যাগিংয়ের জন্য এটি কাজ করবে।
